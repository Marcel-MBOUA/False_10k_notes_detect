        ...
        if st.button("Analyser le billet"):
            with st.spinner('Analyse en cours...'):
                ...
                with torch.no_grad():
                    ...
                    if prediction[0] == 1:
                        st.success(f"Le billet semble être une coupure authentique de 10,000 XAF, avec un seuil de {round(prob, 2)}")
                    else:
                        st.error(f"Alerte : Probabilité de contrefaçon détectée !, avec un seuil de {round(float(prob), 2)}")